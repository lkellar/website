...
def generateHtml():
    ...
    with doc.head:
        ...
        tags.link(rel="alternate", title="Changelog Feed", type="application/json", href="https://lkellar.org/changelog/feed.json")
        ...
        with tags.main():
            ...
            for entry in data:
                ...
                with article_content:
                    tags.h2(f'{entry["title"]} - {entry["date"].split("T")[0]}', id=f'{entry["title"]} - {entry["date"]}'.replace(' ', ''.lower()))
                ...
                articles.append((f'{entry["title"]} - {entry["date"]}'.replace(' ', ''.lower()), list_content.render(), entry["date"], entry['title']))
